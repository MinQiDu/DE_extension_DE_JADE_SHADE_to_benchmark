...
void FileOutput(
	...
	// 輸出整合結果到 SHADE_integrated_fitness.txt
	...
	///* 出圖(每一 run 分開) */
	//// 輸出每 run 運行的 fitness 到 SHADE_fitness_cvg.txt
	...
	/* 出圖(所有 run 平均) */
	ofstream SHADE_fitness_cvg_avg_file("SHADE_fitness_cvg_avg" + to_string(func_id) + "_iter" + to_string(iter) + "_dim" + to_string(dim) + "_archive_" + (archive_flag ? "true" : "false") + ".txt");
	if (!SHADE_fitness_cvg_avg_file) {
	...
	ofstream avg_plot_file("plot_avg_SHADE_func" + to_string(func_id) + "_iter" + to_string(iter) + "_dim" + to_string(dim) + "_archive_" + (archive_flag ? "true" : "false") + ".plt");
	if (!avg_plot_file) {
		cerr << "Error opening average plot file." << endl;
		return;
	}
	avg_plot_file << "set terminal pngcairo size 1200,600 enhanced font 'Verdana,10'\n";
	avg_plot_file << "set output 'SHADE_avg_cvg_plot" << "_func" << func_id << "_iter" << iter << "_dim" << dim << "_archive_" << (archive_flag ? "true" : "false") << ".png'\n";
	avg_plot_file << "set title 'SHADE Average Convergence Plot" << " (f" << func_id << ", iter" << iter << ", dim" << dim << ", archive: " << (archive_flag ? "true" : "false") << ")'\n";
	avg_plot_file << "set xlabel 'Iteration'\n";
	avg_plot_file << "set ylabel 'Best Fitness'\n";
	...
	avg_plot_file << "plot 'SHADE_fitness_cvg_avg" << to_string(func_id) << "_iter" << to_string(iter) << "_dim" << to_string(dim) << "_archive_" << (archive_flag ? "true" : "false") << ".txt' with lines title 'Average Fitness'\n";
	avg_plot_file.close();
}
