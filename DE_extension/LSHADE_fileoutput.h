...
void FileOutput(
	...
	/* 出圖(所有 run 平均) */
	ofstream SHADE_fitness_cvg_avg_file("SHADE_fitness_avg_cvg" + to_string(func_id) + "_iter" + to_string(iter) + "_dim" + to_string(dim) + "_archive_" + (archive_flag ? "true" : "false") + ".txt");
	...
	avg_plot_file << "plot 'SHADE_fitness_avg_cvg" << to_string(func_id) << "_iter" << to_string(iter) << "_dim" << to_string(dim) << "_archive_" << (archive_flag ? "true" : "false") << ".txt' with lines title 'Average Fitness'\n";
	avg_plot_file.close();
}#pragma once
