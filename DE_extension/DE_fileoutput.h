...
void FileOutput(
	...
	output_file << "DE | func_id = " << func_id << " | iter = " << iter << " | dim = " << dim << " | pop_size = " << pop_size
		<< " | CR = " << _CR << " | F = " << _F << endl;
	...
	integrated_output << "DE | func_id = " << func_id << "| iter = " << iter << " | dim = " << dim << " | pop_size = " << pop_size
		<< " | CR = " << _CR << " | F = " << _F << endl;
	...
	/* 出圖(所有 run 平均) */
	ofstream avg_cvg_file("DE_fitness_avg_cvg" + to_string(func_id) + "_iter" + to_string(iter) + "_dim" + to_string(dim) + ".txt");
	if (!avg_cvg_file) {
	...
	ofstream avg_plot_file("plot_avg_DE_func" + to_string(func_id) + "_iter" + to_string(iter) + "_dim" + to_string(dim) + ".plt");
	if (!avg_plot_file) {
