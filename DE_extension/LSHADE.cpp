...
void algo_LSHADE::RunALG(int _iter, int _dim, int _pop_size, double _mCR, double _mF, double _H, double _p, int _func_id, bool _archive_flag)
{
	int run = 51; // 設定運行次數
	...
	for (int r = 0; r < run; ++r)
	{
		...
		init_pop_size = _pop_size; // 記住初始群體大小
		pop_size = _pop_size;	   // 當前群體大小
		...
		nfes = 0;				   // 初始化評估次數
		mnfes = dim * 10000;	   // 最大評估次數，這裡設定為 dim * 10000
...
void algo_LSHADE::Init()
{
	...
	max_archive_size = static_cast<int>(r_archive * pop_size); // 計算 archive list A 的最大大小
	A.reserve(max_archive_size); // Reserve space for A
...
void algo_LSHADE::Mutation()
{
	...
	for (int i = 0; i < pop_size; ++i)
	{
		...
		int p_num = std::max(2, static_cast<int>(std::ceil(p_i * pop_size)));
...
void algo_LSHADE::Determination()
{
	sF.reserve(pop_size);
...
void algo_LSHADE::ParaAdaptation()
{
	// Maintain size of A <= max_archive_size
	while (A.size() > max_archive_size)
	{
...
void algo_LSHADE::LPSR()
{
	// Linear Population Size Reduction
	const int MAX_NFE = mnfes; // Maximum number of function evaluations
	const int NFE = nfes;      // Current number of function evaluations
	const int Ninit = init_pop_size;
	const int Nmin = min_pop_size;

	int next_pop_size = static_cast<int>(round(
		((double)(Nmin - Ninit) / (double)MAX_NFE) * (double)NFE + (double)Ninit));
}